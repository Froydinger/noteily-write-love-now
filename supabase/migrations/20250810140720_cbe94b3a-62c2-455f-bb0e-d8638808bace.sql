...
DECLARE
  v_exists BOOLEAN;
BEGIN
  SELECT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'cleanup-old-deleted-notes-daily') INTO v_exists;
  IF v_exists THEN
    PERFORM cron.unschedule('cleanup-old-deleted-notes-daily');
  END IF;
